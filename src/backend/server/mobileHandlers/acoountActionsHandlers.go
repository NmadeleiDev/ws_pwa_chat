...
func	SignUpHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		userData.SecretHash = utils.GenerateSecret(*userData)
		...
		if utils.RefreshRequestSessionKeyCookie(w, *userData) {
			utils.SendDataResponse(w, userData.SecretHash)
		}
	}
}
...
func	SignInHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		userData.SecretHash = utils.GenerateSecret(*userData)
		if postgres.SetUserSecret(*userData) && utils.RefreshRequestSessionKeyCookie(w, *userData) {
			utils.SendDataResponse(w, userData.SecretHash)
		} else {
			utils.SendFailResponse(w)
		}
